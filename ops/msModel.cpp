...
bool msModel::LoadFromMemory(uint8_t * ptr, size_t size)
{
		ops::files::MemFile loader(ptr, size, false);
        ...
        loader.seek(0, SEEK_END);
        long fileSize = loader.tell();
        loader.seek(0, SEEK_SET);

        char id[10];
        loader.read(id, sizeof(char)*10);
        if (strncmp(id, "MS3D000000", 10) != 0)
        {
                loader.close();
                // "This is not a valid MS3D file format!"
        ...
        int version;
        loader.read(&version, sizeof(int));
        ...
        // vertices
        unsigned short numVertices;
        loader.read(&numVertices, sizeof(unsigned short));
        m_vertices.resize(numVertices);
        for (i = 0; i < numVertices; i++)
        {
                loader.read(&m_vertices[i].flags, sizeof(unsigned char));
                loader.read(&m_vertices[i].vertex, sizeof(float)*3);
                loader.read(&m_vertices[i].boneId, sizeof(char));
                loader.read(&m_vertices[i].referenceCount, sizeof(unsigned char));
        }

        // triangles
        unsigned short numTriangles;
        loader.read(&numTriangles, sizeof(unsigned short));
        m_triangles.resize(numTriangles);
        for (i = 0; i < numTriangles; i++)
        {
                loader.read(&m_triangles[i].flags, sizeof(unsigned short));
                loader.read(m_triangles[i].vertexIndices, sizeof(unsigned short)*3);
                loader.read(m_triangles[i].vertexNormals, sizeof(float) * 3 * 3);
                loader.read(m_triangles[i].s, sizeof(float)* 3);
                loader.read(m_triangles[i].t, sizeof(float)*3);
                loader.read(&m_triangles[i].smoothingGroup, sizeof(unsigned char));
                loader.read(&m_triangles[i].groupIndex, sizeof(unsigned char));
        ...
        // groups
        unsigned short numGroups;
        loader.read(&numGroups, sizeof(unsigned short));
        m_groups.resize(numGroups);
        for (i = 0; i < numGroups; i++)
        {
                loader.read(&m_groups[i].flags, sizeof(unsigned char));
                loader.read(m_groups[i].name, sizeof(char)* 32);

                unsigned short numGroupTriangles;
                loader.read(&numGroupTriangles, sizeof(unsigned short));
                m_groups[i].triangleIndices.resize(numGroupTriangles);
                if (numGroupTriangles > 0)
                        loader.read(&m_groups[i].triangleIndices[0], sizeof(unsigned short)* numGroupTriangles);

                loader.read(&m_groups[i].materialIndex, sizeof(char));
        }

        // materials
        unsigned short numMaterials;
        loader.read(&numMaterials, sizeof(unsigned short));
        m_materials.resize(numMaterials);
        for (i = 0; i < numMaterials; i++)
        {
                loader.read(m_materials[i].name, sizeof(char)* 32);
                loader.read(&m_materials[i].ambient, sizeof(float)* 4);
                loader.read(&m_materials[i].diffuse, sizeof(float)* 4);
                loader.read(&m_materials[i].specular, sizeof(float)* 4);
                loader.read(&m_materials[i].emissive, sizeof(float)* 4);
                loader.read(&m_materials[i].shininess, sizeof(float));
                loader.read(&m_materials[i].transparency, sizeof(float));
                loader.read(&m_materials[i].mode, sizeof(unsigned char));
                loader.read(m_materials[i].texture, sizeof(char)* MAX_TEXTURE_FILENAME_SIZE);
                loader.read(m_materials[i].alphamap, sizeof(char)* MAX_TEXTURE_FILENAME_SIZE);
        ...
        // animation
        loader.read(&m_animationFps, sizeof(float));
        if (m_animationFps < 1.0f)
                m_animationFps = 1.0f;
        loader.read(&m_currentTime, sizeof(float));
        loader.read(&m_totalFrames, sizeof(int));

        // joints
        unsigned short numJoints;
        loader.read(&numJoints, sizeof(unsigned short));
        m_joints.resize(numJoints);
        for (i = 0; i < numJoints; i++)
        {
                loader.read(&m_joints[i].flags, sizeof(unsigned char));
                loader.read(m_joints[i].name, sizeof(char)* 32);
                loader.read(m_joints[i].parentName, sizeof(char)* 32);
                loader.read(m_joints[i].rot, sizeof(float)* 3);
                loader.read(m_joints[i].pos, sizeof(float)* 3);

                unsigned short numKeyFramesRot;
                loader.read(&numKeyFramesRot, sizeof(unsigned short));
                m_joints[i].rotationKeys.resize(numKeyFramesRot);

                unsigned short numKeyFramesPos;
                loader.read(&numKeyFramesPos, sizeof(unsigned short));
                m_joints[i].positionKeys.resize(numKeyFramesPos);
                ...
                for (j = 0; j < numKeyFramesRot; j++)
                {
                        loader.read(&m_joints[i].rotationKeys[j].time, sizeof(float));
                        loader.read(&m_joints[i].rotationKeys[j].key, sizeof(float)* 3);
                        m_joints[i].rotationKeys[j].time *= m_animationFps;
                }
                ...
                for (j = 0; j < numKeyFramesPos; j++)
                {
                        loader.read(&m_joints[i].positionKeys[j].time, sizeof(float));
                        loader.read(&m_joints[i].positionKeys[j].key, sizeof(float)* 3);
                        m_joints[i].positionKeys[j].time *= m_animationFps;
                }
        }

        // comments
        long filePos = loader.tell();
        if (filePos < fileSize)
        {
                int subVersion = 0;
                loader.read(&subVersion, sizeof(int));
                if (subVersion == 1)
                {
                        int numComments = 0;
                        size_t commentSize = 0;
                        ...
                        // group comments
                        loader.read(&numComments, sizeof(int));
                        for (i = 0; i < numComments; i++)
                        {
                                int index;
                                loader.read(&index, sizeof(int));
                                std::vector<char> comment;
                                loader.read(&commentSize, sizeof(size_t));
                                comment.resize(commentSize);
                                if (commentSize > 0)
                                        loader.read(&comment[0], sizeof(char)* commentSize);
                                if (index >= 0 && index < (int) m_groups.size())
                                        m_groups[index].comment = comment;
                        }
        ...
        if (filePos < fileSize)
        {
                ...
                if (subVersion == 2)
                {
                        for (int i = 0; i < numVertices; i++)
                        {
                                loader.read(&m_vertices[i].boneIds[0], sizeof(char)* 3);
                                loader.read(&m_vertices[i].weights[0], sizeof(unsigned char)* 3);
                                loader.read(&m_vertices[i].extra, sizeof(unsigned int));
                        }
                }
                else if (subVersion == 1)
                {
        ...
        if (filePos < fileSize)
        {
                ...
                if (subVersion == 1)
                {
                        for (int i = 0; i < numJoints; i++)
                        {
                                loader.read(&m_joints[i].color, sizeof(float)* 3);
                        }
                }
                else
                {
        ...
        if (filePos < fileSize)
        {
                ...
                if (subVersion == 1)
                {
                        loader.read(&m_jointSize, sizeof(float));
                        loader.read(&m_transparencyMode, sizeof(int));
                        loader.read(&m_alphaRef, sizeof(float));
                }
                else
                {
                        //"Unknown subversion for model extra %d\n", subVersion);
